...
uint32_t ACCD1 = 0, ACCD2 = 0;  // raw MS5611 pressure and temperature data
...
float Acceleromter::getPitch()
{
	return pitch;
}

float Acceleromter::getRoll()
{
	return roll;
}

float Acceleromter::getYaw()
{
	return yaw;
}

float* Acceleromter::getQuaternion()
{
	return q;
}

void Acceleromter::setup()
{
	Wire1.begin();
...
void Acceleromter::loop()
{
	...
	if (delt_t > 500) { // update LCD once per half-second independent of read rate
		...
		pitch *= 180.0f / PI;
		yaw *= 180.0f / PI;
		yaw -= 3.4f; // Declination at Los Altos, California is ~13 degrees 13 minutes on 2020-07-19
		roll *= 180.0f / PI;

		/*
		Serial.print(180 * atan(ax / sqrt(ay * ay + az * az)) / M_PI);
		Serial.print("  |  ");
		Serial.println(pitch);
		*/

		//Serial.print(180 * atan(ay / sqrt(ax * ax + az * az)) / M_PI);
		//Serial.print("  |  ");
		/*
		Serial.print(roll);
		Serial.print("  |  ");
...
void Acceleromter::readAccelData(int16_t* destination)
{
...
void Acceleromter::readGyroData(int16_t* destination)
{
...
void Acceleromter::readMagData(int16_t* destination)
{
	uint8_t rawData[6];  // x/y/z gyro register data stored here
...
int16_t Acceleromter::readTempData()
{
...
void Acceleromter::selftestLSM9DS1()
{
...
void Acceleromter::accelgyrocalLSM9DS1(float* dest1, float* dest2)
{
...
void Acceleromter::magcalLSM9DS1(float* dest1)
{
...
void Acceleromter::writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
...
uint8_t Acceleromter::readByte(uint8_t address, uint8_t subAddress)
{
...
void Acceleromter::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t* dest)
{
	Wire1.beginTransmission(address);   // Initialize the Tx buffer
	Wire1.write(subAddress);            // Put slave register address in Tx buffer
	//  Wire.endTransmission(I2C_NOSTOP);  // Send the Tx buffer, but send a restart to keep connection alive
...
void Acceleromter::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
{
...
float* Acceleromter::getEulers()
{
	...
	anotherYaw = anotherYaw * 180.0 / PI, 2;

	anotherYaw = atan2(mx, my) * 180.0 / PI;
