/*
  ...
  * Egg: (peripheral hosting "EggCallback"-Service)
  ...
  // toDo: test "Connected to peripheral" + "Advertise" at the same time
...
static const DemoScanParam_t my_scan_param = {ble::scan_interval_t(160), ble::scan_window_t(160), ble::scan_duration_t(0), false};
...
{
public:
  GapDemo(BLE &ble, events::EventQueue &event_queue) : _ble(ble),
                                                       _gap(ble.gap()),
...
private:
  ...
  void scan()
  {
    const DemoScanParam_t &scan_params = my_scan_param;
...
private:
  ...
  virtual void onAdvertisingReport(const ble::AdvertisingReportEvent &event)
  {
    ...
    const address_t eggAdress = address_t(eggArray); //93:06:de:ea:cc:58
    const address_t &addr = event.getPeerAddress();
    ...
    while (adv_parser.hasNext())
    {
      ...
      //const address_t &addr = event.getPeerAddress();
      const address_t &addr = eggAdress;
  ...
  virtual void
  onAdvertisingEnd(const ble::AdvertisingEndEvent &event)
  {
