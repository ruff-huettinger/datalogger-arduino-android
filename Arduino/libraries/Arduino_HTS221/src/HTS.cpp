...
void HTS221Class::end()
{
	...
	data = i2cRead(HTS221_CTRL1_REG);
	data &= ~0x80;
	i2cWrite(HTS221_CTRL1_REG, data);
	*/
...
float HTS221Class::readTemperature(int units)
{
	...
	Serial.println("one shot hang");

	// trigger one shot
	i2cWrite(HTS221_CTRL2_REG, 0x01);

	Serial.println("trigger hang");
...
float HTS221Class::getSlope() {
	return _hts221TemperatureSlope;

}

float HTS221Class::getZero() {
	return _hts221TemperatureZero;
